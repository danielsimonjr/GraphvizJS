...
The **`core/`** directory (repo root, Node-only, no DOM) owns all Graphviz work **and all pure DOT language tooling**: `render.ts` (DOT→SVG + syntax validation via `@hpcc-js/wasm`), `normalize-svg.ts` (pure-string viewBox/padding rewrite — no DOM `getBBox`), `export-png.ts` (`@resvg/resvg-js`), `export-pdf.ts` (headless vector PDF), the `export.ts` orchestrator (`exportDiagram`), plus the relocated pure language modules: `scan-dot.ts` (literal-aware span scanner), `dot-vocab.ts` (`DOT_KEYWORDS`/`DOT_ATTRIBUTES`), `structure-lint.ts` (`structuralDiagnostics`), `format.ts` (`formatDot`), and `validate.ts` (`validateDiagram` — the oracle combining syntax + structural). Semantic lint adds `dot-catalog.ts` (`DOT_ATTRIBUTE_CATALOG`/`findAttribute` — attribute contexts + value domains, transcribed from the Graphviz attrs table), `dot-colors.ts` (`DOT_COLORS`/`isColorAttribute`), `edit-distance.ts` (`editDistance`/`nearest` — bounded Levenshtein for "did you mean" suggestions), `semantic-lint.ts` (`semanticDiagnostics` — invalid-value, invalid-color, wrong-context, duplicate-attribute, and undefined-cluster checks, folded into `validateDiagram`), and `apply-fixes.ts` (`applyFixes` — applies a diagnostic's `fix` to source, used by the editor's quick-fix actions and CLI `validate --fix`). Both the Electron main process and the **`cli/`** binary (`graphvizjs`) consume it. Types live in `core/types.ts`.
...
The main process hosts the headless `core/` and registers the IPC handlers the renderer calls: `render:svg`, `render:validate` (returns the combined `{ syntax, structural }` `DiagramDiagnostics` from `validateDiagram`, `structural` now including semantic-lint findings with optional `code`/`fix`), `export:render`, `dot:format` (`formatDot`), and `dot:vocabulary` (DOT keywords/attributes plus `attributeValues` — the enum value domains — and `colors`, both from the attribute catalog), alongside file-dialog/filesystem, `electron-store` key-value, external-file-watch, and native-menu channels. All are exposed to the renderer through the preload script (`electron/preload.ts`) as `window.graphviz`, wrapped by `src/platform/`. `menu:action`/`file:changed` are push channels (main→renderer).
